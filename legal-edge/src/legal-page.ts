...
const navigation: { key: LegalPageKey; label: string }[] = [
...
export function legalPageKey(pathname: string): LegalPageKey | null {
...
export function legalPage(key: LegalPageKey): string {
...
<body>
  <header>
    <a class="brand" href="/" aria-label="DuelWords AV home"><picture><source media="(prefers-color-scheme: dark)" srcset="${brandLogoDark}"><img src="${brandLogo}" alt="DuelWords AV" width="1020" height="230"></picture></a>
    <nav aria-label="Legal pages">${navigation.map((item) => `<a${key === item.key ? ' aria-current="page"' : ''} href="/${item.key}/">${item.label}</a>`).join('')}</nav>
  </header>
  <main>
    <h1>${page.title}</h1>
    <p class="lead">${page.lead}</p>
    <p class="updated">Effective ${updatedAt}</p>
